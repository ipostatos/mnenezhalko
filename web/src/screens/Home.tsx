...
export function Home({
  ...
  const tiles: Tile[] = [
    ...
    // клубов может быть несколько, в том числе в одном городе (B7), поэтому
    // плашка ведёт на экран со списком, а не сразу в единственный чат.
    // Число клубов приходит с сервера — плашку рисуем, только когда есть куда вести
    ...((health?.clubs ?? 0) > 0
      ? ([
          {
            icon: 'flower',
            tone: '#DD93C2',
            title: 'Книжные клубы',
            sub: 'Читаем вместе — чаты клубов по городам',
            route: { name: 'clubs' },
          },
        ] as Tile[])
      : []),
    ...
    // плашка «Добавить книгу» убрана (просьба user 5.08.2026): она дублировала
    // промо-баннер «Поделись книгой» вверху главной, оба вели в один экран `add`
