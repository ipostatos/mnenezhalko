...
export const env = {
  ...
  /**
   * Донаты (Telegram Stars). ВЫКЛЮЧЕНЫ по умолчанию: механика звёзд вызывает у
   * людей больше вопросов, чем приносит пользы, и до отдельного продуктового
   * решения пользовательских входов быть не должно (B5, ТЗ 5.08.2026).
   * Платёжный код цел и включается одним `DONATIONS_ENABLED=1`.
   */
  donations: process.env.DONATIONS_ENABLED === '1',
