...
# ── прогон проверки ─────────────────────────────────────────
# Через `bash` явно: файлы приезжают из архива релиза без exec-бита, и прямой
# запуск падал «Permission denied» (поймано на проде при установке таймера).
report=$(bash "$CHECK" 2>&1)
check_rc=$?
echo "$report"

# 🔥 Проверка, которая НЕ СМОГЛА выполниться, — это инцидент, а не тишина.
# Именно так watchdog и обманул: отчёт оказался пустым, строк «🔴» в нём не было,
# и он честно написал «норма, молчу», не зная о проде ничего. Сломанный
# мониторинг, который выглядит как здоровый прод, хуже отсутствующего.
if [ "$check_rc" -gt 1 ] || [ -z "$(printf '%s' "$report" | tr -d '[:space:]')" ]; then
  report="$report
  🔴 проверка наблюдения не выполнилась (код $check_rc) — о состоянии прода ничего не известно"
  # инцидентом, а не сбоем юнита: сигнал людям даёт сообщение, а не failed в systemctl
  check_rc=1
fi
